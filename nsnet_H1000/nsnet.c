// *****************************************************************************
// 	THIS CODE WAS AUTOMATICALLY GENERATED ON 2024-03-07 16:16:40
// *****************************************************************************
...
void allocnn() {
FILE *fp;
READMAT(tensor_fc1bias, 1000, "fc1bias.bin", float32_t)
...
READMAT(tensor_onnxMatMul_164, 257000, "onnxMatMul_164.bin", float32_t)
READMAT(tensor_onnxGRU_182, 3000000, "onnxGRU_182.bin", float32_t)
READMAT(tensor_onnxGRU_183, 3000000, "onnxGRU_183.bin", float32_t)
READMAT(tensor_onnxGRU_184, 6000, "onnxGRU_184.bin", float32_t)
READMAT(tensor_onnxGRU_202, 3000000, "onnxGRU_202.bin", float32_t)
READMAT(tensor_onnxGRU_203, 3000000, "onnxGRU_203.bin", float32_t)
READMAT(tensor_onnxGRU_204, 6000, "onnxGRU_204.bin", float32_t)
READMAT(tensor_onnxMatMul_205, 600000, "onnxMatMul_205.bin", float32_t)
...
float32_t tensor_fc1MatMul_output_0[1 * 1000];
...
#pragma omp reduction(temp: +)
        ...
        tensor_fc1MatMul_output_0[i*1000 + j] = temp;
    }
}

#ifdef COMPILER_BENCHMARK
BENCHMARK("tensor_fc1MatMul", 1514)
#endif
...
for(int i=0; i<1; i++) {
    for(int j=0; j<1000; j++) {
        printf("%f, ", tensor_fc1MatMul_output_0[i*1000+j]);
    }
    printf("\n");
}
...
#ifdef CONNECTED_OUTPUT
float32_t tensor_fc1Add_output_0[1000];
#undef CONNECTED_OUTPUT
#endif
...
tensor_fc1Add_output_0[i1*1000 + i2*1] = tensor_fc1bias[i2*1] + tensor_fc1MatMul_output_0[i1*1000 + i2*1];
}
}
}
...
#ifdef CONNECTED_OUTPUT
float tensor_gru1Transpose_output_0[1000];
#undef CONNECTED_OUTPUT
#endif
...
tensor_gru1Transpose_output_0[i0*(1*1000)+i2*(1)] = tensor_fc1Add_output_0[i1*1000 + i2*1];
}
}
}
...
#ifdef CONNECTED_OUTPUT
float tensor_gru1GRU_output_0[1000];
#undef CONNECTED_OUTPUT
#endif
...
#pragma omp reduction(a: +)
        for(int j=0; j<1000; j++) {
            a[i] += tensor_onnxGRU_182[(1000*1000) + i*1000 + j] * tensor_gru1Transpose_output_0[j];
        }
        a[i] += tensor_onnxGRU_184[1000 + i];
    }
...
#pragma omp reduction(b: +)
        for(int j=0; j<1000; j++) {
            b[i] += tensor_onnxGRU_183[(1000*1000) + i*1000 + j] * tensor_hidden1[j];
        }
        b[i] += tensor_onnxGRU_184[4*1000 + i];
    }
...
#pragma omp reduction(c: +)
        for(int j=0; j<1000; j++) {
            c[i] += tensor_onnxGRU_182[i*1000 + j] * tensor_gru1Transpose_output_0[j];
        }
...
#pragma omp reduction(d: +)
        for(int j=0; j<1000; j++) {
            d[i] += tensor_onnxGRU_183[i*1000 + j] * tensor_hidden1[j];
        }
        d[i] += tensor_onnxGRU_184[3*1000 + i];
    }
...
#pragma omp reduction(e: +)
        for(int j=0; j<1000; j++) {
            e[i] += tensor_onnxGRU_182[(2*1000*1000) + i*1000 + j] * tensor_gru1Transpose_output_0[j];
        }
        e[i] += tensor_onnxGRU_184[2*1000 + i];
    }
...
#pragma omp reduction(f: +)
        for(int j=0; j<1000; j++) {
            f[i] += tensor_onnxGRU_183[(2*1000*1000) + i*1000 + j] * tensor_hidden1[j];
        }
        f[i] += tensor_onnxGRU_184[5*1000 + i];
    }
...
#ifdef COMPILER_BENCHMARK
BENCHMARK("tensor_/gru1/GRU", 12028000)
#endif
...
#ifdef CONNECTED_OUTPUT
float tensor_gru2GRU_output_0[1000];
#undef CONNECTED_OUTPUT
#endif
...
#pragma omp reduction(a: +)
        for(int j=0; j<1000; j++) {
            a[i] += tensor_onnxGRU_202[(1000*1000) + i*1000 + j] * tensor_gru1Squeeze_output_0[j];
        }
        a[i] += tensor_onnxGRU_204[1000 + i];
    }
...
#pragma omp reduction(b: +)
        for(int j=0; j<1000; j++) {
            b[i] += tensor_onnxGRU_203[(1000*1000) + i*1000 + j] * tensor_hidden[j];
        }
        b[i] += tensor_onnxGRU_204[4*1000 + i];
    }
...
#pragma omp reduction(c: +)
        for(int j=0; j<1000; j++) {
            c[i] += tensor_onnxGRU_202[i*1000 + j] * tensor_gru1Squeeze_output_0[j];
        }
...
#pragma omp reduction(d: +)
        for(int j=0; j<1000; j++) {
            d[i] += tensor_onnxGRU_203[i*1000 + j] * tensor_hidden[j];
        }
        d[i] += tensor_onnxGRU_204[3*1000 + i];
    }
...
#pragma omp reduction(e: +)
        for(int j=0; j<1000; j++) {
            e[i] += tensor_onnxGRU_202[(2*1000*1000) + i*1000 + j] * tensor_gru1Squeeze_output_0[j];
        }
        e[i] += tensor_onnxGRU_204[2*1000 + i];
    }
...
#pragma omp reduction(f: +)
        for(int j=0; j<1000; j++) {
            f[i] += tensor_onnxGRU_203[(2*1000*1000) + i*1000 + j] * tensor_hidden[j];
        }
        f[i] += tensor_onnxGRU_204[5*1000 + i];
    }
...
#ifdef COMPILER_BENCHMARK
BENCHMARK("tensor_/gru2/GRU", 12028000)
#endif
...
#ifdef CONNECTED_OUTPUT
float tensor_gru2Transpose_output_0[1000];
#undef CONNECTED_OUTPUT
#endif
...
tensor_gru2Transpose_output_0[i0*(1*1000)+i2*(1)] = tensor_gru2Squeeze_output_0[i1*1000 + i2*1];
}
}
}
...
#ifdef COMPILER_BENCHMARK
BENCHMARK("tensor_fc3MatMul", 2600)
#endif

// *****************************************************************************
// 	THIS CODE WAS AUTOMATICALLY GENERATED ON 2024-03-13 18:29:27
// *****************************************************************************
...
void allocnn() {
FILE *fp;
READMAT(tensor_fc1bias, 400, "fc1bias.bin", float32_t)
READMAT(tensor_fc3bias, 600, "fc3bias.bin", float32_t)
READMAT(tensor_fc4bias, 600, "fc4bias.bin", float32_t)
READMAT(tensor_fc2bias, 1000, "fc2bias.bin", float32_t)
READMAT(tensor_onnxMatMul_164, 400000, "onnxMatMul_164.bin", float32_t)
READMAT(tensor_onnxGRU_182, 480000, "onnxGRU_182.bin", float32_t)
READMAT(tensor_onnxGRU_183, 480000, "onnxGRU_183.bin", float32_t)
READMAT(tensor_onnxGRU_184, 2400, "onnxGRU_184.bin", float32_t)
READMAT(tensor_onnxGRU_202, 480000, "onnxGRU_202.bin", float32_t)
READMAT(tensor_onnxGRU_203, 480000, "onnxGRU_203.bin", float32_t)
READMAT(tensor_onnxGRU_204, 2400, "onnxGRU_204.bin", float32_t)
READMAT(tensor_onnxMatMul_205, 240000, "onnxMatMul_205.bin", float32_t)
READMAT(tensor_onnxMatMul_206, 360000, "onnxMatMul_206.bin", float32_t)
READMAT(tensor_onnxMatMul_207, 600000, "onnxMatMul_207.bin", float32_t)
}
...
float32_t tensor_fc1MatMul_output_0[1 * 400];
...
#pragma omp reduction(temp: +)
        for(int32_t k=0; k<1000; k++) {
            int32_t index1 = i*1000+k;
            int32_t index2 = k*400+j;
            temp += tensor_onnxMatMul_0[index1] * tensor_onnxMatMul_164[index2];
        }
        tensor_fc1MatMul_output_0[i*400 + j] = temp;
    }
}
...
for(int i=0; i<1; i++) {
    for(int j=0; j<400; j++) {
        printf("%f, ", tensor_fc1MatMul_output_0[i*400+j]);
    }
    printf("\n");
}
...
#ifdef CONNECTED_OUTPUT
float32_t tensor_fc1Add_output_0[400];
#undef CONNECTED_OUTPUT
#endif
...
tensor_fc1Add_output_0[i1*400 + i2*1] = tensor_fc1bias[i2*1] + tensor_fc1MatMul_output_0[i1*400 + i2*1];
}
}
}
...
#ifdef CONNECTED_OUTPUT
float tensor_gru1Transpose_output_0[400];
#undef CONNECTED_OUTPUT
#endif
...
tensor_gru1Transpose_output_0[i0*(1*400)+i2*(1)] = tensor_fc1Add_output_0[i1*400 + i2*1];
}
}
}
...
#ifdef CONNECTED_OUTPUT
float tensor_gru1GRU_output_0[400];
#undef CONNECTED_OUTPUT
#endif
...
#pragma omp reduction(a: +)
        for(int j=0; j<400; j++) {
            a[i] += tensor_onnxGRU_182[(400*400) + i*400 + j] * tensor_gru1Transpose_output_0[j];
        }
        a[i] += tensor_onnxGRU_184[400 + i];
    }
...
#pragma omp reduction(b: +)
        for(int j=0; j<400; j++) {
            b[i] += tensor_onnxGRU_183[(400*400) + i*400 + j] * tensor_hidden1[j];
        }
        b[i] += tensor_onnxGRU_184[4*400 + i];
    }
...
#pragma omp reduction(c: +)
        for(int j=0; j<400; j++) {
            c[i] += tensor_onnxGRU_182[i*400 + j] * tensor_gru1Transpose_output_0[j];
        }
...
#pragma omp reduction(d: +)
        for(int j=0; j<400; j++) {
            d[i] += tensor_onnxGRU_183[i*400 + j] * tensor_hidden1[j];
        }
        d[i] += tensor_onnxGRU_184[3*400 + i];
    }
...
#pragma omp reduction(e: +)
        for(int j=0; j<400; j++) {
            e[i] += tensor_onnxGRU_182[(2*400*400) + i*400 + j] * tensor_gru1Transpose_output_0[j];
        }
        e[i] += tensor_onnxGRU_184[2*400 + i];
    }
...
#pragma omp reduction(f: +)
        for(int j=0; j<400; j++) {
            f[i] += tensor_onnxGRU_183[(2*400*400) + i*400 + j] * tensor_hidden1[j];
        }
        f[i] += tensor_onnxGRU_184[5*400 + i];
    }
...
#ifdef CONNECTED_OUTPUT
float tensor_gru2GRU_output_0[400];
#undef CONNECTED_OUTPUT
#endif
...
#pragma omp reduction(a: +)
        for(int j=0; j<400; j++) {
            a[i] += tensor_onnxGRU_202[(400*400) + i*400 + j] * tensor_gru1Squeeze_output_0[j];
        }
        a[i] += tensor_onnxGRU_204[400 + i];
    }
...
#pragma omp reduction(b: +)
        for(int j=0; j<400; j++) {
            b[i] += tensor_onnxGRU_203[(400*400) + i*400 + j] * tensor_hidden[j];
        }
        b[i] += tensor_onnxGRU_204[4*400 + i];
    }
...
#pragma omp reduction(c: +)
        for(int j=0; j<400; j++) {
            c[i] += tensor_onnxGRU_202[i*400 + j] * tensor_gru1Squeeze_output_0[j];
        }
...
#pragma omp reduction(d: +)
        for(int j=0; j<400; j++) {
            d[i] += tensor_onnxGRU_203[i*400 + j] * tensor_hidden[j];
        }
        d[i] += tensor_onnxGRU_204[3*400 + i];
    }
...
#pragma omp reduction(e: +)
        for(int j=0; j<400; j++) {
            e[i] += tensor_onnxGRU_202[(2*400*400) + i*400 + j] * tensor_gru1Squeeze_output_0[j];
        }
        e[i] += tensor_onnxGRU_204[2*400 + i];
    }
...
#pragma omp reduction(f: +)
        for(int j=0; j<400; j++) {
            f[i] += tensor_onnxGRU_203[(2*400*400) + i*400 + j] * tensor_hidden[j];
        }
        f[i] += tensor_onnxGRU_204[5*400 + i];
    }
...
#ifdef CONNECTED_OUTPUT
float tensor_gru2Transpose_output_0[400];
#undef CONNECTED_OUTPUT
#endif
...
tensor_gru2Transpose_output_0[i0*(1*400)+i2*(1)] = tensor_gru2Squeeze_output_0[i1*400 + i2*1];
}
}
}
...
#ifdef CONNECTED_OUTPUT
float32_t tensor_fc2MatMul_output_0[1 * 1000];
#undef CONNECTED_OUTPUT
#endif
...
for(int i=0; i<1; i++) {
    for(int j=0; j<1000; j++) {
        printf("%f, ", tensor_fc2MatMul_output_0[i*1000+j]);
    }
    printf("\n");
}
...
#ifdef CONNECTED_OUTPUT
float32_t tensor_fc2Add_output_0[1000];
#undef CONNECTED_OUTPUT
#endif
...
tensor_fc2Add_output_0[i1*1000 + i2*1] = tensor_fc2bias[i2*1] + tensor_fc2MatMul_output_0[i1*1000 + i2*1];
}
}
}
...
float32_t ex = exp(tensor_fc2Add_output_0[i1*1000 + i2*1]);
tensor_163[i1*1000 + i2*1] = ex / (1.0f + ex);
}
}
}

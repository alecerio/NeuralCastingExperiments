// *****************************************************************************
// 	THIS CODE WAS AUTOMATICALLY GENERATED ON 2024-03-13 18:28:16
// *****************************************************************************
...
READMAT(tensor_fc2bias, 600, "fc2bias.bin", float32_t)
READMAT(tensor_onnxMatMul_164, 240000, "onnxMatMul_164.bin", float32_t)
...
READMAT(tensor_onnxMatMul_207, 360000, "onnxMatMul_207.bin", float32_t)
}
...
#ifdef CONNECTED_OUTPUT
float32_t tensor_fc2MatMul_output_0[1 * 600];
#undef CONNECTED_OUTPUT
#endif
...
#pragma omp reduction(temp: +)
        ...
        tensor_fc2MatMul_output_0[i*600 + j] = temp;
    }
}
...
for(int i=0; i<1; i++) {
    for(int j=0; j<600; j++) {
        printf("%f, ", tensor_fc2MatMul_output_0[i*600+j]);
    }
    printf("\n");
}
...
#ifdef CONNECTED_OUTPUT
float32_t tensor_fc2Add_output_0[600];
#undef CONNECTED_OUTPUT
#endif
...
tensor_fc2Add_output_0[i1*600 + i2*1] = tensor_fc2bias[i2*1] + tensor_fc2MatMul_output_0[i1*600 + i2*1];
}
}
}
...
float32_t ex = exp(tensor_fc2Add_output_0[i1*600 + i2*1]);
tensor_163[i1*600 + i2*1] = ex / (1.0f + ex);
}
}
}

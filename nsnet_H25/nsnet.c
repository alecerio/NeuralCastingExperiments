// *****************************************************************************
// 	THIS CODE WAS AUTOMATICALLY GENERATED ON 2024-03-07 15:42:49
// *****************************************************************************
...
void allocnn() {
FILE *fp;
READMAT(tensor_fc1bias, 25, "fc1bias.bin", float32_t)
...
READMAT(tensor_onnxMatMul_164, 6425, "onnxMatMul_164.bin", float32_t)
READMAT(tensor_onnxGRU_182, 1875, "onnxGRU_182.bin", float32_t)
READMAT(tensor_onnxGRU_183, 1875, "onnxGRU_183.bin", float32_t)
READMAT(tensor_onnxGRU_184, 150, "onnxGRU_184.bin", float32_t)
READMAT(tensor_onnxGRU_202, 1875, "onnxGRU_202.bin", float32_t)
READMAT(tensor_onnxGRU_203, 1875, "onnxGRU_203.bin", float32_t)
READMAT(tensor_onnxGRU_204, 150, "onnxGRU_204.bin", float32_t)
READMAT(tensor_onnxMatMul_205, 15000, "onnxMatMul_205.bin", float32_t)
...
float32_t tensor_fc1MatMul_output_0[1 * 25];
...
#pragma omp reduction(temp: +)
        for(int32_t k=0; k<257; k++) {
            ...
            int32_t index2 = k*25+j;
            temp += tensor_onnxMatMul_0[index1] * tensor_onnxMatMul_164[index2];
        }
        tensor_fc1MatMul_output_0[i*25 + j] = temp;
    }
}

#ifdef COMPILER_BENCHMARK
BENCHMARK("tensor_fc1MatMul", 539)
#endif
...
for(int i=0; i<1; i++) {
    for(int j=0; j<25; j++) {
        printf("%f, ", tensor_fc1MatMul_output_0[i*25+j]);
    }
    printf("\n");
}
...
#ifdef CONNECTED_OUTPUT
float32_t tensor_fc1Add_output_0[25];
#undef CONNECTED_OUTPUT
#endif
...
tensor_fc1Add_output_0[i1*25 + i2*1] = tensor_fc1bias[i2*1] + tensor_fc1MatMul_output_0[i1*25 + i2*1];
}
}
}
...
#ifdef CONNECTED_OUTPUT
float tensor_gru1Transpose_output_0[25];
#undef CONNECTED_OUTPUT
#endif
...
tensor_gru1Transpose_output_0[i0*(1*25)+i2*(1)] = tensor_fc1Add_output_0[i1*25 + i2*1];
}
}
}
...
#ifdef CONNECTED_OUTPUT
float tensor_gru1GRU_output_0[25];
#undef CONNECTED_OUTPUT
#endif
...
#pragma omp reduction(a: +)
        for(int j=0; j<25; j++) {
            a[i] += tensor_onnxGRU_182[(25*25) + i*25 + j] * tensor_gru1Transpose_output_0[j];
        }
        a[i] += tensor_onnxGRU_184[25 + i];
    }
...
#pragma omp reduction(b: +)
        for(int j=0; j<25; j++) {
            b[i] += tensor_onnxGRU_183[(25*25) + i*25 + j] * tensor_hidden1[j];
        }
        b[i] += tensor_onnxGRU_184[4*25 + i];
    }
...
#pragma omp reduction(c: +)
        for(int j=0; j<25; j++) {
            c[i] += tensor_onnxGRU_182[i*25 + j] * tensor_gru1Transpose_output_0[j];
        }
...
#pragma omp reduction(d: +)
        for(int j=0; j<25; j++) {
            d[i] += tensor_onnxGRU_183[i*25 + j] * tensor_hidden1[j];
        }
        d[i] += tensor_onnxGRU_184[3*25 + i];
    }
...
#pragma omp reduction(e: +)
        for(int j=0; j<25; j++) {
            e[i] += tensor_onnxGRU_182[(2*25*25) + i*25 + j] * tensor_gru1Transpose_output_0[j];
        }
        e[i] += tensor_onnxGRU_184[2*25 + i];
    }
...
#pragma omp reduction(f: +)
        for(int j=0; j<25; j++) {
            f[i] += tensor_onnxGRU_183[(2*25*25) + i*25 + j] * tensor_hidden1[j];
        }
        f[i] += tensor_onnxGRU_184[5*25 + i];
    }
...
#ifdef COMPILER_BENCHMARK
BENCHMARK("tensor_/gru1/GRU", 8200)
#endif
...
#ifdef CONNECTED_OUTPUT
float tensor_gru2GRU_output_0[25];
#undef CONNECTED_OUTPUT
#endif
...
#pragma omp reduction(a: +)
        for(int j=0; j<25; j++) {
            a[i] += tensor_onnxGRU_202[(25*25) + i*25 + j] * tensor_gru1Squeeze_output_0[j];
        }
        a[i] += tensor_onnxGRU_204[25 + i];
    }
...
#pragma omp reduction(b: +)
        for(int j=0; j<25; j++) {
            b[i] += tensor_onnxGRU_203[(25*25) + i*25 + j] * tensor_hidden[j];
        }
        b[i] += tensor_onnxGRU_204[4*25 + i];
    }
...
#pragma omp reduction(c: +)
        for(int j=0; j<25; j++) {
            c[i] += tensor_onnxGRU_202[i*25 + j] * tensor_gru1Squeeze_output_0[j];
        }
...
#pragma omp reduction(d: +)
        for(int j=0; j<25; j++) {
            d[i] += tensor_onnxGRU_203[i*25 + j] * tensor_hidden[j];
        }
        d[i] += tensor_onnxGRU_204[3*25 + i];
    }
...
#pragma omp reduction(e: +)
        for(int j=0; j<25; j++) {
            e[i] += tensor_onnxGRU_202[(2*25*25) + i*25 + j] * tensor_gru1Squeeze_output_0[j];
        }
        e[i] += tensor_onnxGRU_204[2*25 + i];
    }
...
#pragma omp reduction(f: +)
        for(int j=0; j<25; j++) {
            f[i] += tensor_onnxGRU_203[(2*25*25) + i*25 + j] * tensor_hidden[j];
        }
        f[i] += tensor_onnxGRU_204[5*25 + i];
    }
...
#ifdef COMPILER_BENCHMARK
BENCHMARK("tensor_/gru2/GRU", 8200)
#endif
...
#ifdef CONNECTED_OUTPUT
float tensor_gru2Transpose_output_0[25];
#undef CONNECTED_OUTPUT
#endif
...
tensor_gru2Transpose_output_0[i0*(1*25)+i2*(1)] = tensor_gru2Squeeze_output_0[i1*25 + i2*1];
}
}
}
...
#ifdef COMPILER_BENCHMARK
BENCHMARK("tensor_fc3MatMul", 650)
#endif

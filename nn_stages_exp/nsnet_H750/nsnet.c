// *****************************************************************************
// 	THIS CODE WAS AUTOMATICALLY GENERATED ON 2024-03-07 16:10:54
// *****************************************************************************
...
void allocnn() {
FILE *fp;
READMAT(tensor_fc1bias, 750, "fc1bias.bin", float32_t)
...
READMAT(tensor_onnxMatMul_164, 192750, "onnxMatMul_164.bin", float32_t)
READMAT(tensor_onnxGRU_182, 1687500, "onnxGRU_182.bin", float32_t)
READMAT(tensor_onnxGRU_183, 1687500, "onnxGRU_183.bin", float32_t)
READMAT(tensor_onnxGRU_184, 4500, "onnxGRU_184.bin", float32_t)
READMAT(tensor_onnxGRU_202, 1687500, "onnxGRU_202.bin", float32_t)
READMAT(tensor_onnxGRU_203, 1687500, "onnxGRU_203.bin", float32_t)
READMAT(tensor_onnxGRU_204, 4500, "onnxGRU_204.bin", float32_t)
READMAT(tensor_onnxMatMul_205, 450000, "onnxMatMul_205.bin", float32_t)
...
float32_t tensor_fc1MatMul_output_0[1 * 750];
...
for(int i=0; i<1; i++) {
    for(int j=0; j<750; j++) {
        printf("%f, ", tensor_fc1MatMul_output_0[i*750+j]);
    }
    printf("\n");
}
...
#ifdef CONNECTED_OUTPUT
float32_t tensor_fc1Add_output_0[750];
#undef CONNECTED_OUTPUT
#endif
...
tensor_fc1Add_output_0[i1*750 + i2*1] = tensor_fc1bias[i2*1] + tensor_fc1MatMul_output_0[i1*750 + i2*1];
}
}
}
...
#ifdef CONNECTED_OUTPUT
float tensor_gru1Transpose_output_0[750];
#undef CONNECTED_OUTPUT
#endif
...
tensor_gru1Transpose_output_0[i0*(1*750)+i2*(1)] = tensor_fc1Add_output_0[i1*750 + i2*1];
}
}
}
...
#ifdef CONNECTED_OUTPUT
float tensor_gru1GRU_output_0[750];
#undef CONNECTED_OUTPUT
#endif
...
#pragma omp reduction(a: +)
        for(int j=0; j<750; j++) {
            a[i] += tensor_onnxGRU_182[(750*750) + i*750 + j] * tensor_gru1Transpose_output_0[j];
        }
        a[i] += tensor_onnxGRU_184[750 + i];
    }
...
#pragma omp reduction(b: +)
        for(int j=0; j<750; j++) {
            b[i] += tensor_onnxGRU_183[(750*750) + i*750 + j] * tensor_hidden1[j];
        }
        b[i] += tensor_onnxGRU_184[4*750 + i];
    }
...
#pragma omp reduction(c: +)
        for(int j=0; j<750; j++) {
            c[i] += tensor_onnxGRU_182[i*750 + j] * tensor_gru1Transpose_output_0[j];
        }
...
#pragma omp reduction(d: +)
        for(int j=0; j<750; j++) {
            d[i] += tensor_onnxGRU_183[i*750 + j] * tensor_hidden1[j];
        }
        d[i] += tensor_onnxGRU_184[3*750 + i];
    }
...
#pragma omp reduction(e: +)
        for(int j=0; j<750; j++) {
            e[i] += tensor_onnxGRU_182[(2*750*750) + i*750 + j] * tensor_gru1Transpose_output_0[j];
        }
        e[i] += tensor_onnxGRU_184[2*750 + i];
    }
...
#pragma omp reduction(f: +)
        for(int j=0; j<750; j++) {
            f[i] += tensor_onnxGRU_183[(2*750*750) + i*750 + j] * tensor_hidden1[j];
        }
        f[i] += tensor_onnxGRU_184[5*750 + i];
    }
...
#ifdef COMPILER_BENCHMARK
BENCHMARK("tensor_/gru1/GRU", 6771000)
#endif
...
#ifdef CONNECTED_OUTPUT
float tensor_gru2GRU_output_0[750];
#undef CONNECTED_OUTPUT
#endif
...
#pragma omp reduction(a: +)
        for(int j=0; j<750; j++) {
            a[i] += tensor_onnxGRU_202[(750*750) + i*750 + j] * tensor_gru1Squeeze_output_0[j];
        }
        a[i] += tensor_onnxGRU_204[750 + i];
    }
...
#pragma omp reduction(b: +)
        for(int j=0; j<750; j++) {
            b[i] += tensor_onnxGRU_203[(750*750) + i*750 + j] * tensor_hidden[j];
        }
        b[i] += tensor_onnxGRU_204[4*750 + i];
    }
...
#pragma omp reduction(c: +)
        for(int j=0; j<750; j++) {
            c[i] += tensor_onnxGRU_202[i*750 + j] * tensor_gru1Squeeze_output_0[j];
        }
...
#pragma omp reduction(d: +)
        for(int j=0; j<750; j++) {
            d[i] += tensor_onnxGRU_203[i*750 + j] * tensor_hidden[j];
        }
        d[i] += tensor_onnxGRU_204[3*750 + i];
    }
...
#pragma omp reduction(e: +)
        for(int j=0; j<750; j++) {
            e[i] += tensor_onnxGRU_202[(2*750*750) + i*750 + j] * tensor_gru1Squeeze_output_0[j];
        }
        e[i] += tensor_onnxGRU_204[2*750 + i];
    }
...
#pragma omp reduction(f: +)
        for(int j=0; j<750; j++) {
            f[i] += tensor_onnxGRU_203[(2*750*750) + i*750 + j] * tensor_hidden[j];
        }
        f[i] += tensor_onnxGRU_204[5*750 + i];
    }
...
#ifdef COMPILER_BENCHMARK
BENCHMARK("tensor_/gru2/GRU", 6771000)
#endif
...
#ifdef CONNECTED_OUTPUT
float tensor_gru2Transpose_output_0[750];
#undef CONNECTED_OUTPUT
#endif
...
tensor_gru2Transpose_output_0[i0*(1*750)+i2*(1)] = tensor_gru2Squeeze_output_0[i1*750 + i2*1];
}
}
}
...
for(int32_t i=0; i<1; i++) {
    for(int32_t j=0; j<600; j++) {
        float32_t temp = 0.0f;
#pragma omp reduction(temp: +)
        for(int32_t k=0; k<750; k++) {
            int32_t index1 = i*750+k;
...
#ifdef COMPILER_BENCHMARK
BENCHMARK("tensor_fc3MatMul", 2100)
#endif
